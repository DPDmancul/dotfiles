...
* History
 ...
 + =inc_append_history= :: Add history lines incrementally (as soon as they are entered).
 + =hist_ignore_space= :: Prevent adding commands with leading spaces to the history.
...
# format = '[]($style)[ $symbol $common_meaning$signal_name$maybe_int ](white bg:red)[]($style)'
